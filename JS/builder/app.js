const { BUILDER, RUNE_DATA, setBuilderState } = window.BuilderState;
const BuilderRunes = window.BuilderRunes;
const BuilderStats = window.BuilderStats;
const BuilderAbility = window.BuilderAbility;
...
function ingestRunesReforged(runes) {
  const parsed = BuilderRunes.parseRunesReforged(runes, RUNE_DATA.runeLookup);
  RUNE_DATA.paths = parsed.paths;
  RUNE_DATA.pathDefaults = parsed.defaults;
  RUNE_DATA.runeLookup = parsed.runeLookup;
}
...
function readNumericStat(entry, base, aliases) {
  return BuilderStats.readNumericStat(entry, base, aliases);
}

function buildMergedItemStats(ddragonStats, cdtbEntry) {
  return BuilderStats.buildMergedItemStats(ddragonStats, cdtbEntry);
}
...
window.BuilderApp = { initBuilder, wireBuilderUiEvents };
